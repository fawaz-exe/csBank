...
const verifyPhone = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Phone is verified successfully !" });
  } catch (error) {
    console.log(error);
    res
      .status(500)
  ...
  try {
    ...
    if (user.verified.email == false) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Email not verified. Please verify and try again",
        });
    }
    if (user.verified.phone == false) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Phone not verified. Please verify and try again",
        });
    }
...
const verifyJwtToken = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "decoded , payload is fetched",
        data: decoded,
      });
  } catch (error) {
    console.log(error);
...
const currentUser = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Fetched current User Details", data });
  } catch (error) {
    console.log(error);
    res
      .status(500)
...
const passwordReset = async (req, res) => {
  try {
    ...
    if (!user.passwordTokenVerified.email) {
      return res
        .status(403)
        .json({ success: false, message: "Password reset not verified" });
    }
